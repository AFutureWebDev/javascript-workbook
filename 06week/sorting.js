...
// Save the return value from the stringsToNumbs function, which is the strNums array as an array of numbers.
const digitsArr = stringsToNumbs(strNums);
// with the same numbers, find the sum of the even values
const evenArr = [];
const isEven = digitsArr.filter(function(num){
    if(num % 2 === 0) {
    evenArr.push(num);
    }
});
...
// When you get to a number in the array, when its added to its index number it equals 512
const atxIdx = digitsArr.findIndex(function(value, index){
...
let weatherStates = weather.map(function(itemValue){
...
const findItem = weather.find(function(item){
    return item['min_temp'] === 15.915;
});

const idealTemp = findItem.id;
